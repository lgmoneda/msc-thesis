...
from models.functions import score_by_period, check_min_sample_periods, \
    initialize_period_dict, fill_right_dict, check_min_sample_periods_dict
...
difference in cases min_leaf < |time_periods| * min_sample_periods (default: 5)
    ...
    (default: True)
    """
    def __init__(self, n_estimators=5, time_column="period", max_depth=5,
                 min_leaf=5, min_sample_periods=100, max_features="auto",
                 bootstrapping=True, period_criterion="avg", n_jobs=-1,
                 multi=True):
    ...
    def fit(self, X, y, sample_weight=None, verbose=False):
        ...
        self.n_estimators_ = []
        if not self.multi:
            self.n_estimators_ = [RandomTimeSplitTree(
                X,
                y,
                ...
                random_state=i) for i in range(self.n_estimators)]
        else:
            self.n_estimators_ = Parallel(
                n_jobs=self.n_jobs, verbose=0)(delayed(RandomTimeSplitTree)(
                    X,
                    y,
                    min_sample_periods=self.min_sample_periods,
                    max_depth=self.max_depth,
    ...
    def feature_importance(self):
        return pd.concat([n_estimator.feature_importance for n_estimator in self.n_estimators_]).groupby(level=0).sum().sort_values(ascending=False)
...
difference in cases min_leaf < |time_periods| * min_sample_periods (default: 5)
    ...
    """
    def __init__(self, X, y, row_indexes = [], time_column="period", max_depth=5,
                 min_leaf=5, max_features="auto", bootstrapping=True,
                 criterion="gini", period_criterion="avg",
                 min_sample_periods=100, sample_weight=None, depth=None,
                 verbose=False, split_verbose=False, random_state=42):
        if len(row_indexes) == 0: row_indexes = np.arange(len(y))
        ### Reindex
        if depth == None:
            depth = 0
            if bootstrapping:
                resampled_X = X.sample(frac=1.0, replace=True,
        ...
        if max_features == "auto":
            ...
            self.max_n_variables = max(int(max_features * len(self.variables)), 1)
        ...
        if self.depth <= self.max_depth and check_min_sample_periods(self.X.loc[self.row_indexes],
                                                                     self.time_column,
    ...
    def create_split(self):
        variables_to_consider = np.random.choice(self.variables,
                                                 self.max_n_variables,
                                                 replace=False)
        ...
        self.left_split = RandomTimeSplitTree(self.X, self.y, self.row_indexes[left_split],
                                              depth=self.depth + 1,
                                              max_features=self.max_features,
                                              bootstrapping=self.bootstrapping,
                                              min_sample_periods=self.min_sample_periods,
        ...
        self.right_split = RandomTimeSplitTree(self.X, self.y, self.row_indexes[right_split],
                                               depth=self.depth + 1,
                                               max_features=self.max_features,
                                               bootstrapping=self.bootstrapping,
                                               min_sample_periods=self.min_sample_periods,
    ...
    def find_better_split(self, variable, variable_idx):
        ...
        #sorted_period_data = self.X.loc[sorted_indexes, self.time_column]
        ...
        right_period_dict = fill_right_dict(sorted_period_data, sorted_y, sorted_weights, right_period_dict)


        for example in range(0, self.n_examples - self.min_leaf):
            ...
            if example < self.min_leaf or x_i == sorted_x[example + 1]:
                continue
            elif not check_min_sample_periods_dict(right_periods_count, self.min_sample_periods) or not check_min_sample_periods_dict(left_periods_count, self.min_sample_periods):
                continue
            if not check_min_sample_periods_dict(right_periods_count, self.min_sample_periods):
                break


            if self.split_verbose: print("Evaluate a split on variable {} at value {}".format(variable, x_i))
            current_score = score_by_period(right_period_dict, left_period_dict,
                                            self.criterion,
    ...
    def predict_row(self, x):
        ...
        # print("Not at depth {}".format(self.depth))
        # print(self.split_variable)
        # print("One split")
    ...
    def splits(self):
        ...
        while not tree.is_leaf:
            print("{}: {}".format(self.split_variable, self.split_example))
